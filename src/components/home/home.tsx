...
export const Home: FunctionComponent = () => {
    ...
    useEffect(() => {
        const keyup = (event: KeyboardEvent) => {
            if (event.key === '4' && !event.repeat) {
                const nextMode = options.mode === Mode.PRODUCE ? Mode.RECOGNIZE : Mode.PRODUCE
    ...
    return <div className={style.home}>
        <div className={style.container}>
            ...
            <p className={style.infoText}>{options.mode === Mode.RECOGNIZE ? 'Identify the interval' : 'Produce the given interval from the base note'}</p>
